...
class PHPExcel_Reader_Excel2007 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	 ...
	 * @throws   Exception
	 * 
	 * mstarproject added by vladimir.fedorov 16/08/2013
	 */
	public function listWorksheetInfo_CorrectAllRows($pFilename)
	{
		// Check if file exists
		if (!file_exists($pFilename)) {
	...
	public function load($pFilename)
	{
		...
		foreach ($rels->Relationship as $rel) {
			switch ($rel["Type"]) {
				...
				case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
					$dir = dirname($rel["Target"]);
					$relsWorkbook = simplexml_load_string($this->_getFromZipArchive($zip, "$dir/_rels/" . basename($rel["Target"]) . ".rels"), 'SimpleXMLElement', LIBXML_COMPACT | LIBXML_PARSEHUGE);  //~ http://schemas.openxmlformats.org/package/2006/relationships");
					$relsWorkbook->registerXPathNamespace("rel", "http://schemas.openxmlformats.org/package/2006/relationships");

					$sharedStrings = array();
					$xpath = self::array_item($relsWorkbook->xpath("rel:Relationship[@Type='http://schemas.openxmlformats.org/officeDocument/2006/relationships/sharedStrings']"));
					$xmlStrings = simplexml_load_string($this->_getFromZipArchive($zip, "$dir/$xpath[Target]"), 'SimpleXMLElement', LIBXML_COMPACT | LIBXML_PARSEHUGE);  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
